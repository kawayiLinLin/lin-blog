...
在 react15以前，都是递归渲染dom树，js是单线程，ui渲染和js执行是互斥的，所以会比较卡

> 帧
>
> 大多数设备的屏幕刷新率为60帧
> 留给每个帧计算的时间（样式计算，布局、绘制）就是16.66ms
> 浏览器帧的流程如下：

输入事件 -> 定时器 -> 开始帧（resize，scroll，media query change）-> requestAnimationFrame -> 布局layout（计算样式、更新布局）-> 绘制paint

剩余时间为空闲时间（16.66ms中的剩余部分），执行idle callback

什么是 fiber

fiber 是一个执行单元，也是一种数据结构

可以适时的让出浏览器的控制权

requestIdleCallback 中注册的任务，会在空闲时间执行，是低优先级任务

requestAnimationFrame 会在帧开始时执行，是高优先级任务
...
// 参数 timeout，我workLoop要一直没机会执行。超时后，浏览器也必须给我执行了
requestIdleCallback(workLoop, { timeout: 1000 })

function wookLoop(deadline) {
    console.log(`本帧的剩余时间：${parseInt(deadline.timeRemaining())}`) // 大约10ms
    // 还有剩余时间，并且还有未完成的任务
    while ((deadline.timeRemaining() > 0 || deadline.didTimeout) && works.length > 0) {
        performUnitOfWork()
    }
    // while 走完了，走这里 ，如果还有，就继续注册 requestIdleCallback，到下面的帧进行执行
    if (works.length > 0) {
        console.log(`只剩下：${parseInt(deadline.timeRemaining())}`)
...
requestIdleCallback 执行有两种可能性

1. 正常情况，16.66ms内多余的时间，会给它执行

2. 很长时间没有交互了（重要工作要做），这个时间会延长
会延长到 50ms（每秒20帧），因为用户 100ms 内的响应都会感觉很流畅
100 / 2 就得 50，浏览器占50ms，requestIdleCallback 占 50ms

这个 requestIdleCallback 只有 chrome 支持，所以 react 利用 MessageChannel 模拟前者，将回调延迟到绘制之后执行
...
port1.sendMessage('给2')
port2.sendMessage('给1')
```

实现 requestIdleCallback 的 polyfill

```js
let frameDeadLine // 一帧的截止时间
let pendingCallback // 等待执行的回调
const channel  = new MessageChannel()
const activeAnimationTime = 1000 / 60 // 16.66ms 每帧的时间
const timeRemaning = () => frameDeadLine - performance.now()

channel.port2.onmessage = function (event) {
    const currentTime = performance.now() // 页面加载时间到现在的时间差
    // 帧的截止时间小于当前时间，就过期了
    const didTimeout = frameDeadLine <= currentTime

    if (didTimeout || timeRemaning() > 0) {
...
window.requestIdleCallback = function (callback, options) {
    ...
    requestAnimationFrame((rafTime) => {
      // rafTime 页面加载时间到现在的时间差，同 performance.now()
      frameDeadLine = rafTime + activeAnimationTime
      pendingCallback = callback
      // 发消息之后，相当于添加了一个宏任务
      // 执行到 channel.port2.onmessage 相当于浏览器空闲了
...
react 每次渲染有两个阶段（reconciliation）协调 render 阶段和 commit 阶段

1. 协调阶段：理解为 diff 阶段，可被中断，会找出所有节点变更，节点新增，删除，属性变更等，被称之为副作用
2. 提交阶段：将上一次计算出的副作用一次性执行，不能被打断

fiber 的遍历是深度优先遍历

1. 从根节点开始，每个节点的遍历规则
2. 先找第一个儿子，没有第一个儿子则此节点遍历完成
3. 这个儿子有弟弟则遍历弟弟
4. 他没有弟弟，则返回父节点标识，完成父节点遍历，如果有叔叔，则遍历叔叔
5. 没有父节点，则遍历结束
...
3. beginWork B1
4. beginWork C1
5. completeUnitOfWork  C1
6. beginWork C2
7. completeUnitOfWork C2
8. completeUnitOfWork B1
9. beignWork B2
10. completeUnitOfWork B2
11. completeUnitOfWork A1
12. completeUnitOfWork root
...
// 下一个工作单元,fiber 也是一个普通的对象
let workingInProgressRoot = {
    stateNode: container, // 对应的DOM节点
    props: { // fiber 的属性
        children: [element]
    }
    // child 指向第一个儿子
...
function commitRoot() {
    ...
    while (currentFiber) { // c1 c2 b1 b2 a1
...
/**
 * 1. 创建此 fiber 的真实 DOM，通过虚拟 DOM 创建对应的 fiber 树结构
 */
function performUnitOfWork(workingInProgressFiber) {
    ...
    while (workingInProgressFiber) {
        ...
        // 否则，将 workingInProgressFiber 置为它父亲，找它父亲的弟弟，也就是它叔叔
        workingInProgressFiber = workingInProgressFiber.return
    }
}

// 创建真实DOM，创建fiber，构建 fiber 树
function beginWork(workingInProgressFiber) {
    ...
    // 创建子 fiber
    let previousFiber
    if (!Array.isArray(workingInProgressFiber.props.children)) return
    workingInProgressFiber.props.children.forEach((child, i) => {
        const childFiber = {
            type: child.type,
            props: child.props,
            return: workingInProgressFiber,
            effectTag: 'PLACEMENT', // 这个 fiber 对应的节点要被插入到页面中去
...
// 构建副作用（需要进行DOM操作，更新，插入，删除等）链
// 每个节点完成时，把儿子的副作用链 + 此节点合并成一个链，然后给到它父亲
// 副作用链之间通过 nextEffect 链接，有 firstEffect 和 lastEffect 两个指针，是一个单向列表
// 跟收班费一样：组员给组长班费，组长加上自己的给班长，班长加上自己的给老师
function completeUnitOfWork(workingInProgressFiber) {
    const returnFiber = workingInProgressFiber.return

    if (returnFiber) {
        if (!returnFiber.firstEffect) {
            // 让自己的第一个有副作用的子节点赋给父亲的
