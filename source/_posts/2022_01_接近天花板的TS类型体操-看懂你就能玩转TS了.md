...
为了实现一个 ts 类型系统的加法器，我模仿 js 写了一个 ts 工具类型库
...
本文代码见：[https://github.com/kawayiLinLin/typescript-lodash/tree/master](https://github.com/kawayiLinLin/typescript-lodash/tree/master)

在线浏览：[https://github1s.com/kawayiLinLin/typescript-lodash/blob/HEAD/lib/number.ts#L423](https://github1s.com/kawayiLinLin/typescript-lodash/blob/HEAD/lib/number.ts#L423)
...
参考：[https://juejin.cn/post/7050893279818317854#heading-8](https://juejin.cn/post/7050893279818317854#heading-8)
...
去掉数组最后一位的实现：[https://juejin.cn/post/7045536402112512007#heading-2](https://juejin.cn/post/7045536402112512007#heading-2)
...
且用在其他工具类型中时，会出现 `类型实例化过深，并可能无限（Type instantiation is excessively deep and possibly infinite）` 的报错，参考 [github issue](https://github.com/microsoft/TypeScript/issues/37613)
...
然后把删掉的部分的长度累计起来，就是最后模拟从右往左匹配到的索引值
...
原理：基于 `Replace`，递归进行替换，替换掉所有 `MatchStr`
...
在参数中传递循环变量 `Offset` （每次传递时加 1，即 `number.IntAddSingle<Offset, 1>`），当循环变量 `Offset` 和循环次数 `Times` 相等时，结束递归
...
>
> [掘金：Ts 高手篇：22 个示例深入讲解 Ts 最晦涩难懂的高级类型工具](https://juejin.cn/post/6994102811218673700)
>
